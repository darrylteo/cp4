...
// code from book website. again comment out 103
...
int main() {
    ...
    while (cin >> n >> m, n) {
        ...
        vector<point> ch1 = CH_Andrew(p1);
        vector<point> ch2 = CH_Andrew(p2);
        bool ok = true;
        // if point inside another CH sure false
        for (int i = 0; i < ch1.size(); ++i) {
            if (insidePolygon(ch1[i], ch2) != -1) {
                ok = false;
                break;
            }
        }
        for (int i = 0; i < ch2.size(); ++i) {
            if (insidePolygon(ch2[i], ch1) != -1) {
                ok = false;
                break;
            }
        }
        // check intersection of all line segments and line of other CH. if intersection in both CHs then false
        for (int i = 0; i < ch1.size(); ++i) {
            for (int j = 0; j < ch2.size(); ++j) {
                point tmp = lineIntersectSeg(ch1[i], ch1[(i + 1) % ch1.size()], ch2[j], ch2[(j + 1) % ch2.size()]);
                if (insidePolygon(tmp, ch1) != -1 && insidePolygon(tmp, ch2) != -1) {
                    ok = false;
                    break;
                }
            }
        ...
        for (int i = 0; i < ch2.size(); ++i) {
            for (int j = 0; j < ch1.size(); ++j) {
                point tmp = lineIntersectSeg(ch2[i], ch2[(i + 1) % ch2.size()], ch1[j], ch1[(j + 1) % ch1.size()]);
                if (insidePolygon(tmp, ch1) != -1 && insidePolygon(tmp, ch2) != -1) {
                    ok = false;
                    break;
                }
