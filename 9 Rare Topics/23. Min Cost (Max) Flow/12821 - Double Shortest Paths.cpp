...
// add edges twice, each with 1 capacity
...
int main() {
    ...
    int V, E, T = 1;
    while (cin >> V >> E) {
        min_cost_max_flow mcmf(V + 1);
        for (int i = 0; i < E; i++) {
            int u, v;
            ll cost1, cost2;
            cin >> u >> v >> cost1 >> cost2;
            mcmf.add_edge(u, v, 1, cost1);
            mcmf.add_edge(u, v, 1, cost1 + cost2);
        }
        mcmf.add_edge(0, 1, 2, 0); // source
        auto [mf, cost] = mcmf.mcmf(0, V);
