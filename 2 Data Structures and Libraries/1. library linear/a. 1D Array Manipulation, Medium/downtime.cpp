...
// see Competitve Programming 4 by Steven Halim page 109
// so you can implement O(logm) updates too
// actually can just put a 1 when start and -1 when end -_- then find max contiguous sum
...
int main() {
    ...
    RURQ rurq(100005); // at least up to 100001
    int n, k;
    cin >> n >> k;
    for (int i = 0; i < n; ++i) {
        int x;
        cin >> x;
        rurq.range_update(x + 1, x + 1000, 1);
    }

    int myMax = 0;
    for (int i = 1; i <= 100005; ++i) {
        myMax = max(myMax, (int)rurq.rsq(i,i));
    }

    cout << ceil((double)myMax/k) << endl;
