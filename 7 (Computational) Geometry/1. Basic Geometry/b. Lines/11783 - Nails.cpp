...
bool liesInBetween(point p, point i, point q) { // check if point i lies on segment pq
    return (p.x <= i.x + EPS && i.x <= q.x + EPS || q.x <= i.x + EPS && i.x <= p.x + EPS) &&
           (p.y <= i.y + EPS && i.y <= q.y + EPS || q.y <= i.y + EPS && i.y <= p.y + EPS);
}
...
int main() {
    ...
    while (cin >> n, n) {
        ...
        vector<line> lines(n);
        for (int i = 0; i < n; ++i) {
            pointsToLine(segments[i].first, segments[i].second, lines[i]);
        }
        vector<int> used(n, 0);
        int nails = 0;
        point a;
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                if (areIntersect(lines[i], lines[j], a)) {
                    if (liesInBetween(segments[i].first, a, segments[i].second) && liesInBetween(segments[j].first, a, segments[j].second)) {
                        ++nails;
                        used[i] = used[j] = 1;
                    }
                }
