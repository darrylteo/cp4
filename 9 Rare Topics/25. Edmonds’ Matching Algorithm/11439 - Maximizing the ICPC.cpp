...
// bsta on possible min weights to match
// each person must have undirected edge to all other persons that are above min threshold
// i.e. he might fight person A and win, then person B. this aggregates all rounds together.
// if we find (2^N / 2) matchings, then we can say that there is a solution
// ref: https://judge.yosupo.jp/submission/51907
...
int main() {
    ...
    for (int cases = 1; cases <= T; ++cases) {
        ...
        cin >> N;
        int W = 1 << N;
        vector<vector<int>> A(W, vector<int>(W));
        set<int> S;
        for (int i = 0; i < W; ++i) {
            for (int j = i + 1; j < W; ++j) {
                cin >> A[i][j];
        ...
        vector<int> st(S.begin(), S.end()); // we will pick one of these as threshold
        int l = 0, r = st.size() - 1;
        int ans = -1;
        while (l <= r) {
            int m = (l + r) / 2;
