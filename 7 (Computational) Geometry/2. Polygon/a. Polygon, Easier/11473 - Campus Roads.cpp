...
int main() {
    ...
    for (int cases = 1; cases <= tc; ++cases) {
        ...
        vector<double> distsOrig = dists;
        double peri = perimeter(P);
        double distSeg = peri / (T - 1);
        double x = P[0].x, y = P[0].y;
        cout << "Road #" << cases << ":\n";
        cout << fixed << setprecision(2) << x << " " << y << "\n";
        int next = 1;
        for (int i = 0; i < T - 2; ++i) {
            double distLeft = distSeg;
            while (distLeft > dists[next - 1]) {
                x = P[next].x;
                y = P[next].y;
                distLeft -= dists[next - 1];
                next++;
            }
            x += distLeft * (P[next].x - P[next - 1].x) / distsOrig[next - 1];
            y += distLeft * (P[next].y - P[next - 1].y) / distsOrig[next - 1];
            dists[next - 1] -= distLeft;
            cout << fixed << setprecision(2) << x << " " << y << "\n";
        }
        cout << fixed << setprecision(2) << P[K - 1].x << " " << P[K - 1].y << "\n";
        cout << "\n";
    }
