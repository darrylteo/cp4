...
// left side is "from" location
// 0 to N - 1. 0 is company, 1 to N - 1 are locations, except last location as we won't go from last location to any other location
// right side is "to" location
// N to 2 * N - 1. N is first location (not company), N + 1 to N + N - 1 are locations, except company as we won't go from any location to company
...
int main() {
    ...
    // source to company, capacity is K, cost is 0
    mcmf.add_edge(s, 0, K, 0);
    // source to other "from" locations, capacity is 1, cost is 0
    for (int i = 1; i < N; i++) {
        mcmf.add_edge(s, i, 1, 0);
    }
    // "from" locations to "to" locations, capacity is 1, cost is given in input
    for (int i = 0; i < N; i++) {
        for (int j = N + i; j < 2 * N; j++) {
            int cost;
            cin >> cost;
            mcmf.add_edge(i, j, 1, cost);
        }
    }
    // "to" locations to sink, capacity is 1, cost is 0
    for (int i = N; i < 2 * N; i++) {
        mcmf.add_edge(i, t, 1, 0);
    }
    auto [mf, total_cost] = mcmf.mcmf(s, t);
