...
int main() {
    ...
    while (cin >> N >> W >> H >> x >> y) {
        ...
        while (N--) {
            int x1, y1, x2, y2;
            cin >> x1 >> y1 >> x2 >> y2;
            vector<point> tmp = cutPolygon(point(x1, y1), point(x2, y2), P);
            if (insidePolygon(p, tmp) == 1) {
                P = tmp;
            } else {
                P = cutPolygon(point(x2, y2), point(x1, y1), P);
            }
        }
        cout << "Case #" << cases++ << ": " << fixed << setprecision(3) << area(P) << '\n';
    }
