...
int main() {
    ...
    while (cin >> n, n) {
        ...
        while (P.size() != 3) {
            double minAngle = 1e9;
            int minIndex = -1;
            // get the pointiest angle
            for (int i = 0; i < P.size(); ++i) {
                double a = angle(P[(i - 1 + P.size()) % P.size()], P[i], P[(i + 1) % P.size()]);
                a = abs(a);
                if (a < minAngle) {
                    minAngle = a;
                    minIndex = i;
            ...
            vector<point> tmpP = P;
            tmpP.erase(tmpP.begin() + minIndex); // try remove it
            // get new pointiest angle
            double minAngle2 = 1e9;
            for (int i = 0; i < tmpP.size(); ++i) {
                double a = angle(tmpP[(i - 1 + tmpP.size()) % tmpP.size()], tmpP[i], tmpP[(i + 1) % tmpP.size()]);
                a = abs(a);
                if (a < minAngle2) minAngle2 = a;
            }
            // if it became more pointy. break.
            if (minAngle2 < minAngle) break;
