...
// code from book website. commented line 103
...
int main() {
    ...
    while (cin >> c >> r >> o) {
        ...
        cout << "Data set " << cases++ << ":\n";
        vector<point> cops, robbers;
        for (int i = 0; i < c; ++i) {
            double x, y;
            cin >> x >> y;
            cops.emplace_back(x, y);
        }
        for (int i = 0; i < r; ++i) {
            double x, y;
            cin >> x >> y;
        ...
        cops = CH_Andrew(cops);
        robbers = CH_Andrew(robbers);
        for (int i = 0; i < o; ++i) {
            double x, y;
            cin >> x >> y;
            point p(x, y);
            if (insidePolygon(p, cops) >= 0 && c > 2) {
                cout << "     Citizen at (" << x << "," << y << ") is safe.\n";
            } else if (insidePolygon(p, robbers) >= 0 && r > 2) {
                cout << "     Citizen at (" << x << "," << y << ") is robbed.\n";
            } else {
                cout << "     Citizen at (" << x << "," << y << ") is neither.\n";
            }
        }
        cout << '\n';
    }
