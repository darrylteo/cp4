...
public:
    ...
    static bigint
    _big_pow(bigint &a, bigint &b) {      // returns the power value between two Big Integers i.e., a^b, ^ -> power
        bigint ans;
...
bool
bigint::is_bigint(std::string s) {                              // Checks if the feeded integer is valid Number or not.
    if (s[0] == '-')
        s.erase(0, 1);
...
std::string
bigint::subtract(std::string str1, std::string str2) {                 // returns arithmetic subtraction of str1-str2
    int str1_len = str1.length();
    int str2_len = str2.length();
    std::string sum = "";
    if (str1 == str2) {
...
std::string
bigint::multiply(std::string str1, std::string str2) {             // return arithmetic multiplication of str1*str2
    bool toAddNeg = false;
    int str1_len = str1.length();
    int str2_len = str2.length();
    std::string ans = "";
...
std::string
bigint::divide(std::string str1, std::string str2) {                   // return arithmetic division of str1/str2
    std::string ans = "";
    if (str2 == "0") {
        return "0";
    } else if (str1 == str2) {
...
std::string
bigint::shortDivide(std::string s1, unsigned long long int divisor) {     // return arithmetic division of str1/str2
    std::string ans;
    int idx = 0;
...
std::string
bigint::pow(std::string str1, std::string str2) {                  // returns str1^str2, ^ -> power, numerically
    if (str2 == "0") {
        return "1";
    } else if (str1 == "0") {
        if (str2[0] == '-')
...
std::string bigint::sqrt(std::string s) {                 // returns sqrt(s), numerically
    ...
    while (1) {
        ...
        if (prev == mid || (maximum(add(square, mid), s) == add(square, mid) && maximum(square, s) == s) ||
            high == low) {
            break;
        }
...
bool
bigint::isPalindrome(std::string s) {                  // checks if the Integer as string, s is Palindrome no. or not
    if (s[0] == '-')
        s.erase(0, 1);
    unsigned long long int beg = 0, end = s.length() - 1;
    while (beg < end) {
...
// found a faster bigint library
// ref: https://github.com/rgroshanrg/bigint/blob/main/Big%20Integer%20Library%20in%20Single%20file/bigint.h
// proof of sum is n+1 th catalan probably has to do with the recurrence relation given... cat(n+1) = (4n+2)/(n+2)*cat(n)
...
int main() {
    ...
    vector<bigint> cat(5002);
    cat[0] = bigint(1);
    for (int i = 1; i <= 5001; i++) {
        bigint num(4 * i - 2), den(i + 1);
        cat[i] = cat[i - 1] * num / den;
    }
