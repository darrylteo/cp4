...
int main() {
    ...
    int ans = B.solve();
    // if perfect match, just let opponent 1st and reply a match.
    // we will complete the game and he will have nothing left to say
    int perfect = (ans * 2 == N);
    vector<vector<int>> g = B.g;
    vector<int> mate = B.mate;
    cout << perfect + 1 << "\n" << flush;
    if (!perfect) { // reply an unmatched person 1st
        for (int i = 0; i < N; i++) {
            if (mate[i] == -1) {
                cout << i + 1 << "\n" << flush;
                break;
    ...
    while (cin >> reply) { // guarantee opponent will reply a 0 and lose
        if (reply == 0) return 0;
        cout << mate[reply - 1] + 1 << "\n" << flush;
    }
