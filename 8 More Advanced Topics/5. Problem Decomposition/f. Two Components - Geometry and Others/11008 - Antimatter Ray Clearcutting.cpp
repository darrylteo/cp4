...
int n, m, memo[1 << 16][200]; // bitset of dead trees, id of collines to try
vector<int> collines;

inline int dfs(int mask, int id) {
    if (__builtin_popcount(mask) >= m) return 0; // shoot finish m trees
    if (id == collines.size()) return 1e9; // did not shoot enough trees
    int &ans = memo[mask][id];
    if (ans != -1) return ans;
    ans = min(1 + dfs(mask | collines[id], id + 1), dfs(mask, id + 1));
    return ans;
}
...
int main() {
    ...
    for (int cases = 1; cases <= T; ++cases) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                set<int> curr;
                curr.insert(i), curr.insert(j);
                for (int k = 0; k < n; ++k) {
                    if (k == i || k == j) continue;
                    if (collinear(points[i], points[j], points[k])) curr.insert(k);
                }
                colines.insert(curr);
            }
        }
        collines.resize(colines.size(), 0);
        int tmpid = 0;
        for (const set<int> &co: colines) {
            int bs = 0;
            for (int c: co) bs |= 1 << c; // 1 indicates this beam i will shoot down tree c
            collines[tmpid++] = bs;
        }
