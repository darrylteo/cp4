...
// idea is best rotation is along one of the sides. 
// we imagine the current side extend to form a line then get dist to furthest left and right vertices and add up
// after some thinking... we should have just checked the sides of this convex hull rather than any combination 
// that would eliminate the need for left and right vertices since every vertex would be same side (given convex nature)
...
int main() {
    ...
    while (cin >> n, n) {
        ...
        for (int i = 0; i < CH.size() - 1; ++i) {
            for (int j = i + 1; j < CH.size() - 1; ++j) {
                double max_dist_cw = 0;
                double max_dist_ccw = 0;
                for (int k = 0; k < CH.size() - 1; ++k) {
                    double dist = distToLine(CH[k], CH[i], CH[j], tmp);
                    if (ccw(CH[i], CH[j], CH[k])) {
                        max_dist_cw = max(max_dist_cw, dist);
                    } else {
                        max_dist_ccw = max(max_dist_ccw, dist);
                    }
                }
                ans = min(ans, max_dist_cw + max_dist_ccw);
            }
        }
