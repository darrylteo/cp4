...
int main() {
    ...
    while (T--) {
        ...
        P.push_back(P[0]);
        vector<point> remaining = P;
        for (int i = 1; i < P.size(); ++i) { // counter-clockwise order
            point A = P[i], B = P[i - 1];
            remaining = cutPolygon(A, B, remaining);
        }
        if (remaining.size() == 0) cout << "0.00" << endl;
        else cout << fixed << setprecision(2) << area(remaining) << endl;
    }
