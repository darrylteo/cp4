...
struct point_i {
    ...
    bool operator<(point_i other) const {         // override < operator
...
int main() {
    ...
    for (int cases = 1; cases <= T; ++cases) {
        ...
        for (int i = 0; i < p.size(); ++i) {
            for (int j = i + 1; j < p.size(); ++j) {
                ...
                for (int k = j + 1; k < p.size(); ++k) {
                    if (collinear(p[i], p[j], p[k])) ++cnt;
                }
                maxCollinear = max(maxCollinear, cnt);
            }
        }
        ...
        if (p.size() == 1) {
            ...
            cout << p.size() << " gnus, out of which a maximum of " << maxCollinear << " are aligned.\n";
        }
    }
